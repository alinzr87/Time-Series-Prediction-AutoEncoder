...
#$%^^^
#$%^^^autoencoder.fit(norm_reshaped_input, norm_reshaped_input, epochs = 200, batch_size = 8, shuffle = False,verbose=0)
...
#$%^^^autoencoder.save('CIF_NetworkWeights/CIF_autoencoder_model.h5')
#$%^^^encoder.save('CIF_NetworkWeights/CIF_encoder_model.h5')
...
##########################################################################################################################
        ...
        input_dim = Input(shape = (ncol, ))
        encoded_input = Input(shape = (encoding_dim, ))
        encoder=load_model('CIF_NetworkWeights/CIF_encoder_model.h5')
...
####################################################################################
        ...
        encoded32=Reshape((1,16))(encoded22)
        
        branch_0 = Conv1D(16,3, strides=1, padding='same',activation='relu',kernel_initializer=glorot_uniform(1))(input_data)
        ...
        branch_4 = Conv1D(64,3, strides=1, padding='same',activation='relu',kernel_initializer=glorot_uniform(1))(branch_1)
        branch_2 = Conv1D(64,3, strides=1, padding='same',activation='relu',kernel_initializer=glorot_uniform(1))(branch_4)
        ...
        callback = ModelCheckpoint(filepath='CIF_NetworkWeights/checkpoint_CIF_03_%i.h5' %y,monitor='val_loss',save_best_only=True,save_weights_only=True)
        model.compile(loss='mean_squared_error', optimizer=Adam(lr=0.00001))
        model.fit({'input_data':train_input,'input_pred':train_pred_norm_auto},y_train,validation_data=[[val_input,val_pred_norm_auto],y_val],callbacks=[callback],batch_size=3,shuffle=True, epochs=200,verbose=0)
        model.load_weights('CIF_NetworkWeights/checkpoint_CIF_03_%i.h5' %y)
#######################################################################################################
        if (num_test==12):
            test_input_temp=np.zeros([1,window_size,1])
            test_input_temp[0]=test_input[0]
            test_pred_auto_temp=np.zeros([1,4])
            test_pred_auto_temp=test_pred_auto
            pred_temp=model.predict({'input_data':test_input_temp , 'input_pred':test_pred_auto_temp})
...
np.savetxt('Output/CIF_Prediction_03.csv',final_predictions, fmt='%1.3f',delimiter=',')
model.summary()
print('-----------------')
print("SMAPE:")
...
#__________________________________________________________________________________________________
#conv1d_1 (Conv1D)               (None, 6, 16)        64          input_data[0][0]
#__________________________________________________________________________________________________
#reshape_1 (Reshape)             (None, 1, 16)        0           dense_2[0][0]
#__________________________________________________________________________________________________
#concatenate_1 (Concatenate)     (None, 7, 16)        0           conv1d_1[0][0]
#                                                                 reshape_1[0][0]
#__________________________________________________________________________________________________
#conv1d_2 (Conv1D)               (None, 7, 32)        1568        concatenate_1[0][0]
#__________________________________________________________________________________________________
#conv1d_3 (Conv1D)               (None, 7, 64)        6208        conv1d_2[0][0]
#__________________________________________________________________________________________________
#conv1d_4 (Conv1D)               (None, 7, 64)        12352       conv1d_3[0][0]
#__________________________________________________________________________________________________
#flatten_1 (Flatten)             (None, 448)          0           conv1d_4[0][0]
#__________________________________________________________________________________________________
#dense_final (Dense)             (None, 6)            2694        flatten_1[0][0]
#==================================================================================================
#Total params: 23,070
